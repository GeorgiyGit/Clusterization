...
{
    public class GaussianMixtureAlgorithmService : IAbstractClusterizationAlgorithmService<AddGaussianMixtureAlgorithmDTO, GaussianMixtureAlgorithmDTO>
    {
        ...
        public GaussianMixtureAlgorithmService(IRepository<GaussianMixtureAlgorithm> repository,
        ...
        {
            var list = await repository.GetAsync(c => c.NumberOfComponents == model.NumberOfComponents);
        ...
        public async Task ClusterDataBackgroundJob(int profileId, int taskId, string userId)
        {
            ...
            try
            {
                ...
                if (profile == null || profile.Algorithm.TypeId != ClusterizationAlgorithmTypes.GaussianMixture) throw new HttpException(localizer[ErrorMessagePatterns.ProfileNotFound], HttpStatusCode.NotFound);
                ...
                double quotasCount = (double)entitiesCount * (double)profile.DimensionCount * (1 + Math.Sqrt(profile.DimensionCount));
                ...
                var clusters = await GaussianMixture(entitiesHelpModels, profile.DimensionalityReductionTechniqueId, profile.DimensionCount, clusterAlgorithm.NumberOfComponents);
        ...
        public async Task<ICollection<GaussianMixtureAlgorithmDTO>> GetAllAlgorithms()
        {
            var algorithms = await repository.GetAsync(includeProperties: $"{nameof(ClusterizationAbstactAlgorithm.Type)}");

            return mapper.Map<ICollection<GaussianMixtureAlgorithmDTO>>(algorithms);
        }
    }
}
