...
@Component({
  selector: 'app-telegram-messages-tasks-list-page',
  templateUrl: './telegram-messages-tasks-list-page.component.html',
  styleUrl: './telegram-messages-tasks-list-page.component.scss'
})
export class TelegramMessagesTasksListPageComponent implements OnInit {
  tasks: IMyMainTask[] = [];
  ...
  ngOnInit(): void {
    this.request.id = this.route.snapshot.params['messageId'];
    this.loadFirst();
  }
  ...
  loadFirst() {
    ...
    this.isLoading = true;
    this.entitiesTasksService.getTelegramMessageTasks(this.request).subscribe(res => {
