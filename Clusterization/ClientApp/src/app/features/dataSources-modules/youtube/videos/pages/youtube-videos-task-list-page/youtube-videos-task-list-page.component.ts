...
@Component({
  selector: 'app-youtube-videos-task-list-page',
  templateUrl: './youtube-videos-task-list-page.component.html',
  styleUrl: './youtube-videos-task-list-page.component.scss'
})
export class YoutubeVideosTaskListPageComponent implements OnInit {
  tasks: IMyMainTask[] = [];
  ...
  ngOnInit(): void {
    this.request.id = this.route.snapshot.params['videoId'];
    this.loadFirst();
  }
  ...
  loadFirst() {
    ...
    this.isLoading = true;
    this.entitiesTasksService.getYoutubeVideoTasks(this.request).subscribe(res => {
