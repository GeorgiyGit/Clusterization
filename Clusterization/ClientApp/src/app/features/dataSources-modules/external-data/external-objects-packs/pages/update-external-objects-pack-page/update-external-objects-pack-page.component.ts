...
import { IUpdateClusterizationWorkspace } from 'src/app/features/clusterization-module/clusterization/workspace/models/requests/updateClusterizationWorkspace';
...
@Component({
  selector: 'app-update-external-objects-pack-page',
  templateUrl: './update-external-objects-pack-page.component.html',
  styleUrl: './update-external-objects-pack-page.component.scss',
  animations: [
...
export class UpdateExternalObjectsPackPageComponent implements OnInit {
  ...
  get formValue() {
    return this.workspaceForm.value as IUpdateExternalObjectsPackRequest;
  }
  ...
  ngOnInit(): void {
    ...
    this.packsService.getFull(this.id).subscribe(res=>{
  ...
  submit() {
    ...
    this.isLoading = true;
    this.packsService.update(model).subscribe(res => {
      this.toaster.success($localize`Пакет зовнішніх даних оновлено`);
