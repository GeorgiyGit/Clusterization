...
@Component({
  selector: 'app-clusterization-profile-tasks-list-page',
  templateUrl: './clusterization-profile-tasks-list-page.component.html',
  styleUrl: './clusterization-profile-tasks-list-page.component.scss'
})
export class ClusterizationProfileTasksListPageComponent implements OnInit {
  tasks: IMyMainTask[] = [];
  ...
  ngOnInit(): void {
    this.request.id = this.route.snapshot.params['profileId'];
    this.loadFirst();
  }
  ...
  loadFirst() {
    ...
    this.isLoading = true;
    this.entitiesTasksService.getProfileTasks(this.request).subscribe(res => {
