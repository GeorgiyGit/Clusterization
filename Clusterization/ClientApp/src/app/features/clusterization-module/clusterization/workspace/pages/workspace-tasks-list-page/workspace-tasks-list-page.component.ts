...
@Component({
  selector: 'app-workspace-tasks-list-page',
  templateUrl: './workspace-tasks-list-page.component.html',
  styleUrl: './workspace-tasks-list-page.component.scss'
})
export class WorkspaceTasksListPageComponent implements OnInit {
  tasks: IMyMainTask[] = [];
  ...
  ngOnInit(): void {
    this.request.id = this.route.snapshot.params['workspaceId'];
    this.loadFirst();
  }
  ...
  loadFirst() {
    ...
    this.isLoading = true;
    this.entitiesTasksService.getWorkspaceTasks(this.request).subscribe(res => {
